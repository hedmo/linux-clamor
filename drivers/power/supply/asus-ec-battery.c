...
static enum power_supply_property asusec_battery_properties[] = {
...
static const unsigned int asusec_battery_prop_offs[] = {
...
static int asusec_battery_get_value(struct asusec_battery_data *priv,
				    enum power_supply_property psp)
{
	int ret, offs;

	if (psp >= ARRAY_SIZE(asusec_battery_prop_offs))
		return -EINVAL;

	offs = asusec_battery_prop_offs[psp];
	if (!offs)
		return -EINVAL;
				       ...
				       union power_supply_propval *val)
{
	...
	default:
		ret = asusec_battery_get_value(priv, psp);
		if (ret < 0)
			return ret;
...
static void asusec_battery_poll_work(struct work_struct *work)
{
	...
	state = asusec_battery_get_value(priv, POWER_SUPPLY_PROP_STATUS);
	if (state < 0)
		return;
...
static const struct power_supply_desc asusec_battery_desc = {
	.name = "asusec-battery",
	.type = POWER_SUPPLY_TYPE_BATTERY,
	.properties = asusec_battery_properties,
	.num_properties = ARRAY_SIZE(asusec_battery_properties),
	.get_property = asusec_battery_get_property,
};
...
static int asusec_battery_probe(struct platform_device *pdev)
{
	...
	memcpy(&priv->psy_desc, &asusec_battery_desc, sizeof(priv->psy_desc));
...
static const struct of_device_id asusec_battery_ids[] = {
	{ .compatible = "asus,ec-battery" },
	{ }
};
MODULE_DEVICE_TABLE(of, asusec_battery_ids);

static struct platform_driver asusec_battery_driver = {
	.driver = {
		.name = "asusec-battery",
		.of_match_table = of_match_ptr(asusec_battery_ids),
